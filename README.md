...
This class is a facade for the package and should be instantiated with a [configuration object](https://github.com/anthwal/flydrive/blob/develop/examples/config.ts).
...
Each driver extends the abstract class [`Storage`](https://github.com/anthwal/flydrive/blob/develop/packages/flydrive/src/Storage.ts). This class will throw an exception for each methods by default. The driver needs to overwrite the methods it supports.
