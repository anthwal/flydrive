...
describe('Storage Manager', () => {
  test('throw exception when no disk name is defined', () => {
    ...
    expect(fn).toThrow(
      'E_INVALID_CONFIG: Make sure to define a default disk name inside config file',
    );
  });
  ...
  test('throw exception when disk config is missing', () => {
    ...
    expect(fn).toThrow(
      'E_INVALID_CONFIG: Make sure to define config for local disk',
    );
  });
  ...
  test('throw exception when disk config doesnt have driver', () => {
    ...
    expect(fn).toThrow(
      'E_INVALID_CONFIG: Make sure to define driver for local disk',
    );
  });
